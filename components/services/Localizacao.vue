<template scoped>
  <div>
    <div id="content-servicos-4" v-scroll="ScrollContent" class="col-md-10 col-lg-12 px-0 d-flex position-relative justify-content-start align-items-center row mx-auto">
      <div :class="{'show-content-servicos-1': ShowContentScroll}" class="conteudo-right hide-content-servicos-1 shadow pb-xs-20 pb-sm-20 pt-xs-40 pt-sm-40 py-lg-20 order-xs-2 order-sm-2 order-lg-1 col-lg-6 px-lg-20">
        ...
        <p class="text-white text-xs-center text-lg-left text-sm-center mt-4">
          Temos acesso a diversos dispositivos e meios para levantamento de todos os dados especificados pelos nossos clientes para confecção de dossiês e localização de pessoas. Atuamos com toda a privacidade e princípios morais com alto índice de satisfação. Entre em contato conosco hoje mesmo!
        </p>
        ...
        <div class="d-flex mt-3 justify-content-xs-center justify-content-sm-center justify-content-lg-start">
          <BtnLightTekoRouter router-to="/servicos/localizacao-de-pessoas/" />
          <BtnLightTekoWhatsApp class="ml-3" />
        </div>
      </div>
      <div :class="{'show-img-servicos-1': ShowImgScroll}" class="col-lg-7 hide-img-servicos-1 px-0">
        <img src="../../assets/img/servicos/bg-localizacao.webp" class="img-fluid shadow" alt="Localização de pessoas">
      </div>
    </div>
...
<script>
import BtnLightTekoRouter from '../buttons/BtnLightTekoRouter.vue'
import BtnLightTekoWhatsApp from '../buttons/BtnLightTekoWhatsApp.vue'
export default {
  components: {
  ...
  mounted () {
    ...
    this.ShowImgScroll = false
  },
  methods: {
    ScrollContent () {
      const Janela = document.body.getBoundingClientRect()
      const ContainerScroll = document.getElementById('content-servicos-4').getBoundingClientRect()
